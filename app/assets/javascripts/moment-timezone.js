  ...
  ************************************/
  ...
  function addZone (packed) {
    ...
    for (i = 0; i < packed.length; i++) {
      ...
      if (zoneName.indexOf("_") > -1) {
        zoneName = zoneName.substring( zoneName.indexOf("_") + 1 );
        zones[zoneName] = zone;
      }
    }
  }
  ...
  function copyZoneWithName (zone, name) {
    ...
    var linkZone = zones[normalizedName] = new Zone();
    linkZone._set(zone);
    linkZone.name = name;

    if (normalizedName.indexOf("_") > -1) {
      normalizedName = normalizedName.substring( normalizedName.indexOf("_") + 1 );
      
      linkZone = zones[normalizedName] = new Zone();
      linkZone._set(zone);
      linkZone.name = name;
    }
  }
